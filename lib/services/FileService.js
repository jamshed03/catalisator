...
class FileService {
	...
	ensureBaseFiles() {
		...
		if (!this.dryRun && !fs.existsSync(outputEntryDir)) {
	...
	resolveStylePath(task, formatter) {
		...
		const baseDir = task.category ? path.join(this.paths.outputBase, task.category) : path.dirname(task.file)

		return path.join(baseDir, `${generatedName}${formatter.extension}`)
	}

	resolveStyleHeader(stylePath, fileName, formatter) {
		if (this.readFile(stylePath)) {
			return this.readFile(stylePath).split('\n\n.')[0]
		}

		const varFile = formatter.variablesFile
		const relVarsPath = path.relative(path.dirname(stylePath), path.join(this.paths.outputBase, varFile)).replace(/\\/g, '/')
		return formatter.formatHeader(fileName, relVarsPath)
	}
}
