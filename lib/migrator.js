...
class TailwindToScssMigrator {
	...
	_setupBaseFiles() {
		...
		if (!fs.existsSync(outputEntryDir)) fs.mkdirSync(outputEntryDir, { recursive: true })

		if (!fs.existsSync(this.paths.outputEntry)) {
			const relVarsPath = path.relative(outputEntryDir, varPath).replace(/\\/g, '/').replace('.scss', '')
			fs.writeFileSync(this.paths.outputEntry, `@use '${relVarsPath}' as *;\n\n.container { max-width: 1280px; margin: 0 auto; }\n`)
		}
	}
	...
	async migrate(targetFile, categoryDir = null) {
		const absTarget = path.resolve(targetFile)
		if (!fs.existsSync(absTarget)) return

		let fileName = path.basename(absTarget, '.tsx')
		...
		let scssContent = fs.existsSync(scssPath) ? fs.readFileSync(scssPath, 'utf8') : this._getScssHeader(scssPath, fileName)
		...
		for (const match of matches.reverse()) {
			...
			const customClass = match[2]
			const twClasses = match[3]

			const translatedRules = await this._translateClasses(twClasses)

			if (translatedRules) {
				console.log(`  -> CSS extrahiert für: .${customClass}`)
				hasChanges = true
		...
		this._saveFiles(absTarget, jsxContent, scssPath, scssContent)
	}
